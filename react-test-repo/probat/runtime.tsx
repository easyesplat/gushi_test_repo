...
// Persisted caches
const CHOICE_STORE  = "abx_choices_v1";            // experiment_id -> { variant }
const VARIANT_STORE = "probat_variant_cache_v1";   // proposalId   -> { assignment + code }
const VARIANT_TTL_MS = 6 * 60 * 60 * 1000;         // 6 hours

type Persisted = {
  experiment_id: string;
  label: string;     // "control" or variant label
  mjs_path?: string; // empty/undefined for control
  code?: string;     // cached .mjs text for variant
...
function getChoice(exp: string): string | null {
  try {
    const all = JSON.parse(localStorage.getItem(CHOICE_STORE) || "{}");
    return all[exp]?.variant ?? null;
  } catch { return null; }
}
...
export async function recordClick(experiment_id: string, variant_id: string, meta?: Record<string, any>) {
  try {
...
export function withExperiment<P extends object>(
  ...
  return function Experimented(props: P & { onClick?: ReactProbat.MouseEventHandler }) {
    ...
    const handleClick: ReactProbat.MouseEventHandler = (e) => {
      props.onClick?.(e);
    ...
    return <V {...(props as P)} onClick={handleClick} />;
  };
}
